  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
      <div className="p-6 pb-0 flex items-center justify-between mb-4">
        <div>
          <h3 className="text-slate-900 dark:text-white font-bold text-lg">Low Stock Alerts</h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Products that need replenishment</p>
        </div>
        <Link 
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900/50 border-y border-slate-100 dark:border-slate-700">
              ...
              <th className="px-6 py-3 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Stock</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {data?.slice(0, 8).map((item) => {
              ...
              return (
                <tr key={item.sku} className="hover:bg-slate-50 dark:bg-slate-900/50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-slate-900 dark:text-white truncate max-w-[200px]">{item.productName}</div>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 font-medium">{item.sku}</td>
                  <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{item.categoryName}</td>
                  <td className="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white text-center">{item.currentQuantity}</td>
                  <td className="px-6 py-4">
